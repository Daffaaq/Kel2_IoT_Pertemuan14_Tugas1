...
const int redPin = D1;   // Pin untuk menghubungkan LED merah
const int greenPin = D2; // Pin untuk menghubungkan LED hijau
const int bluePin = D3;  // Pin untuk menghubungkan LED biru
...
void reconnect()
{
  while (!client.connected())
  {
    ...
    if (client.connect(macAddr.c_str()))
    {
      ...
      client.subscribe("kelompok2/rgbled");
    }
    else
    {
...
void callback(char *topic, byte *payload, int length)
{
  ...
  if (String(topic) == "kelompok2/rgbled")
  {
...
void loop()
{
  ...
  if (now - lastMeasure > 5000)
  {
    ...
    client.publish("kelompok2/temperature", temperatureTemp);
    client.publish("kelompok2/humidity", humidityTemp);
    client.publish("kelompok2/light", lightLevelTemp);
  }
}